...
(function (context) {
  ...
  TheGraph.Clipboard.copy = function (graph, keys) {
    ...
    for (i = 0, len = keys.length; i < len; i++) {
      ...
      num = graph.nodes.filter(element => element.component == node.component).length;
      newNode.id = newNode.metadata.couchDbDoc.name.concat("-" + num);
      newNode.label = newNode.id;
      newNode.metadata.label = newNode.id;
  ...
  TheGraph.Clipboard.paste = function (graph) {
    ...
    for (i = 0, len = clipboardContent.nodes.length; i < len; i++) {
      ...
      // add copied member to the modelstore
      var bdeMs = document.querySelector('bde-model-store');
      var copy = bdeMs.copyMember(meta.couchDbDoc['_id']);
      bdeMs.addMember(meta.couchDbDoc['_id'], meta.couchDbDoc.name, copy.inslots, copy.outslots, meta);
