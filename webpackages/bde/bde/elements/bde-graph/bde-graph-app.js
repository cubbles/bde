(function (context) {
  ...
  TheGraph.config.app = {
    container: {
      className: "the-graph-app style-scope bde-graph",
      name: "app"
    },
    canvas: {
      ref: "canvas",
      className: "app-canvas style-scope bde-graph"
    },
    svg: {
      className: "app-svg style-scope bde-graph"
    },
    svgGroup: {
      className: "view style-scope bde-graph"
    },
    graph: {
      ref: "graph"
    },
    ...
    modal: {
      className: "context style-scope bde-graph"
    }
  };
  ...
  TheGraph.App = React.createFactory( React.createClass({
    ...
    onTrack: function (event) {
        ...
        if (undefined == event.ddx || undefined == event.ddy) {
            return;
        }
    ...
    triggerFit: function (event) {
      var fit = TheGraph.findFit(this.props.graph, this.bgCanvas.clientWidth, this.bgCanvas.clientHeight);
      this.setState({
        x: fit.x,
        y: fit.y,
        scale: fit.scale
    ...
    keyUp: function (event) {
      ...
      if (TheGraph.metaKeyPressed) {
        ...
        if (TheGraph.deleteKeyPressed) {
            TheGraph.deleteKeyPressed = false;
        }
    },
